...
public class IntStreamOperations {
    ...
    public static void main(String[] args) {
        ...
        // or IntStream.of(3,10,6,1,4,8,2,5,9,7)
        IntStream.of(values)
                 // Terminal Operation
        ...
        System.out.printf("Min: %d%n", IntStream.of(values).min().getAsInt()); // OptionalInt min():
        ...
        System.out.printf("%nSum via reduce method: %d%n", IntStream.of(values).reduce(1, (x,y)->x*y));

        // even values displayed displayed in sorted order
        System.out.printf("%nEven values displayed in sorted order: ");
        IntStream.of(values)
                .filter(value -> value%2==0) // IntPredicate, execute test method, stateless operation
                .sorted() // stateful operation: need to know all of values
                .forEach(value -> System.out.printf("%d ", value));
        System.out.println();
        ...
        // each of the intermediate stream operations result in another intermediate stream
        IntStream.of(values)
                 .filter(value -> value % 2 != 0)
                 .map(value -> value * 10) // IntUnaryOperator
                 .sorted()
                 .forEach(value -> System.out.printf("%d ", value));
        System.out.println();
